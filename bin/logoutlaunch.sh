...
mon_wh=$(niri msg -j outputs | jq -r 'to_entries[0].value.logical | "\(.width) \(.height) \(.scale)"')
x_mon=$(echo "$mon_wh" | cut -d' ' -f1)
y_mon=$(echo "$mon_wh" | cut -d' ' -f2)
hypr_scale=$(echo "$mon_wh" | cut -d' ' -f3 | awk '{printf "%d", $1 * 100}')
