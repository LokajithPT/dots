...
get_hashmap()
{
    ...
    for wallSource in "$@"; do
        ...
        hashMap=$(find "${wallSource}" -type f \( -iname "*.gif" -o -iname "*.jpg" -o -iname "*.jpeg" -o -iname "*.png" -o -iname "*.mp4" -o -iname "*.webm" -o -iname "*.mkv" \) -exec "${hashMech}" {} + | sort -k2)
        if [ -z "${hashMap}" ] ; then
...
# fallbacks for non-Hyprland sessions (e.g. niri)
export hypr_border="${hypr_border:-0}"
export hypr_width="${hypr_width:-2}"
